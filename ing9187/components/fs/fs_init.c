...
//#define LFS_CFG_BLOCK_COUNT     100


#define LITTLEFS_PHYS_ADDR (0x0056000)
...
{
//printf("%s,partition=%d,offset=0x%x,size=%d\r\n",__func__,partition,*offset,size);

#ifdef EXTERN_FLASH_W25Q
read_extend_flash(*offset, buf, size);
#else
...
static int littlefs_block_write(int partition, uint32_t *offset, const uint8_t *buf, uint32_t size)
{
  ...
  //  printf("%s,partition=%d,offset=0x%x,size=%d\r\n",__func__,partition,*offset,size);
#ifdef EXTERN_FLASH_W25Q
	int ret = write_extend_flash(*offset, buf,  size);
#else
	int ret = write_flash(LITTLEFS_PHYS_ADDR + *offset, (unsigned char *)buf, size);
#endif
...
//	read_extend_flash(offset,temp_data,sizeof(temp_data));
//	for ( i = 0 ;i < 16; i++)
//		printf("%02x-",*((uint8_t *)temp_data+i));
//	printf("\r\n");
    return LFS_ERR_OK;
}
